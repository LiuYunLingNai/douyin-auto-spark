...
function result_encrypt(long_str, num = null) {
    ...
    for (let i = 0; i < long_str.length / 3 * 4; i++) {
        ...
        switch (key) {
            case 0:
                var temp_int = (long_int & constant["0"]) >> 18;
                result += constant["str"].charAt(temp_int);
                break;
            case 1:
                var temp_int = (long_int & constant["1"]) >> 12;
                result += constant["str"].charAt(temp_int);
                break;
            case 2:
                var temp_int = (long_int & constant["2"]) >> 6;
                result += constant["str"].charAt(temp_int);
                break;
            case 3:
                var temp_int = long_int & 63;
                result += constant["str"].charAt(temp_int);
                break;
            default:
                break;
...
function sign(url_search_params, user_agent, sign_args) {
    /**
...
// ESM exports (patched for douyin-id-spark)
