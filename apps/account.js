...
async function setupInput(e) {
  ...
  try {
    if (session.step === 'name') {
      ...
      await e.reply('第 2/3 步：请上传包含完整 Cookie JSON 数组的 .txt 文件，也可直接粘贴。')
...
async function accountList(e) {
  ...
  for (const account of accounts) {
    ...
    lines.push(`- ${account.name}（${targets.length} 个续火目标）`)
  }
...
async function saveCookies(e, session, cookieText) {
  ...
  await saveSetupSession(e.user_id, 'messageTemplate', { ...session.draft, cookies })
  const skipped = ignored ? `已忽略 ${ignored} 条无法使用的 Cookie。\n` : ''
  await e.reply(`${skipped}第 3/3 步：发送消息模板；回复“默认”使用全局默认消息。支持 {{friend}}、{{yiyan}} 等占位符。`)
}
