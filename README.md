...
1. 本账号的别名。
2. 点击“扫码获取 Cookie”，打开抖音聊天页中的登录二维码，用抖音 App 扫码；登录成功后 Cookie 会自动填入。若二维码过期或扫码后状态没有刷新，点击“刷新二维码”重新获取。若抖音触发安全验证，可改用 Cookie-Editor 导出的完整 Cookie JSON 数组，直接粘贴或选择 `.txt` 文件。
